...
export async function generateMetadata({ params }: Props) {
  ...
  if (post) {
    const metadata = {
      title: `${(post.title as string) || "Single Post Page"} | ${siteName}`,
      author: authorName,
      robots: {
        index: true,
        follow: true,
        ...
        <div className="container">
          <div className="-mx-4 flex flex-wrap justify-center">
            <div className="w-full px-4">
              <FadeIn delay={0.1}>
                <div className="relative z-20 mb-[60px] h-[300px] overflow-hidden rounded md:h-[400px] lg:h-[500px]">
                  <Image
                    src={(post.coverImage as string) || "/images/blog/default.jpg"}
                    alt="image"
                    width={1288}
                    height={500}
                    ...
                    <div className="flex flex-wrap items-center p-4 pb-4 sm:p-8">
                      <div className="mb-4 mr-5 flex items-center md:mr-10">
                        <div className="mr-4 h-10 w-10 overflow-hidden rounded-full">
                          <Image
                            src={(post.authorImage as string) || "/images/blog/author-default.jpg"}
                            alt="image"
                            className="w-full"
                            width={40}
                            height={40}
                      ...
                      <div className="mb-4 flex items-center">
                        <p className="mr-5 flex items-center text-sm font-medium text-white md:mr-6">
                          ...
                          {format(new Date((post.date as string) || new Date()), "dd MMM yyyy")}
                        </p>
                  ...
                  <div>
                    ...
                    <div className="-mx-4 mb-8 flex flex-wrap">
                      ...
                      {posts.slice(0, 3).map((blog, i) => (
                        <PopularArticle
                          key={i}
                          image={(blog?.coverImage as string) || "/images/blog/default.jpg"}
                          title={((blog?.title as string) || "").slice(0, 30)}
                          name={(blog?.author as string) || "Anonymous"}
                        />
                      ))}
                    </div>
          ...
          <div className="-mx-4 flex flex-wrap">
            <FadeIn delay={0.2} className="mt-14 w-full px-4">
              <h2 className="relative pb-5 text-2xl font-semibold text-dark dark:text-white sm:text-[28px]">
                Related Articles
              </h2>
              <span className="mb-10 inline-block h-[2px] w-20 bg-primary"></span>
            ...
            {posts.slice(0, 3).map((blog, key) => (
              <div key={key} className="w-full px-4 md:w-2/3 lg:w-1/2 xl:w-1/3">
                <SingleBlog blog={{ ...blog, date: (blog.date as string) || new Date().toISOString() }} />
              </div>
            ))}
          </div>
        </div>
